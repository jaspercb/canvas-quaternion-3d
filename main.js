...
Quaternion.prototype.qMul = function(b) {
	return new Quaternion(this.w * b.w - this.x * b.x - this.y * b.y - this.z * b.z, this.w * b.x +
		this.x * b.w + this.y * b.z - this.z * b.y, this.w * b.y - this.x * b.z + this.y * b.w + this.z *
		b.x, this.w * b.z + this.x * b.y - this.y * b.x + this.z * b.w);
};
...
Quaternion.prototype.getSize = function() {
	return Math.sqrt(Math.pow(this.w, 2) + Math.pow(this.x, 2) + Math.pow(this.y, 2) + Math.pow(this.z,
		2));
};
...
Camera.prototype.rotateAroundDistance = function(rotation, distance) {
	...
	this.pos = this.pos.add(this.orientation.inverse().qMul(new Quaternion(0, 0, 0, 1).qMul(this.orientation))
		.mul(distance));
	this.orientation = rotation.qMul(this.orientation);
	this.pos = this.pos.sub(this.orientation.inverse().qMul(new Quaternion(0, 0, 0, 1).qMul(this.orientation))
		.mul(distance));
};
/*Camera.prototype.rotateAroundDistance = function(rotation, distance){
...
Sphere.prototype.draw = function(canvasContext) {
	if (this.shouldDraw) {
		...
		canvasContext.arc(this.drawCoords[0][0], this.drawCoords[0][1], this.drawRadius, 0, 2 * Math.PI,
			false);
...
var mouseMoveListener = function(e) {
	if (mouseDown) {
		...
		if (e.shiftKey) {
			...
			camera.rotateAroundDistance(new Quaternion(1, 0.002 * dMouseY, -0.002 * dMouseX, 0).normalize(),
				cameraDistance);
		}
	}
	oldMouseX = e.pageX;
...
var mouseScrollListener = function(e) {
	...
	camera.pos = camera.pos.add(new Quaternion(0, 0, 0, 1).applyRotation(camera.orientation.inverse())
		.mul(oldCameraDistance - cameraDistance));
};
var mouseDown = false;
var oldMouseX = 0;
var oldMouseY = 0;
var camera = new Camera(new Quaternion(0, 0, 0, -5), new Quaternion(1, 0, 0, 0), new Quaternion(0,
	0, 0, 0));
...
//stars
for (var i = 0; i < 400; i++) {
	objects.push(new Sphere(new ConstantPath(new Quaternion(0, (Math.random() - 0.5) * 10, (Math.random() -
		0.5) * 10, (Math.random() - 0.5) * 10)), 0.01, "white"));
}
...
for (var i = -5; i <= 5; i += 10) {
	for (var j = -5; j <= 5; j += 10) {
		objects.push(new Line(new ConstantPath(new Quaternion(0, j, -i, i)), new ConstantPath(new Quaternion(
			0, j, i, i)), "red", 2));
		objects.push(new Line(new ConstantPath(new Quaternion(0, i, j, -i)), new ConstantPath(new Quaternion(
			0, i, j, i)), "red", 2));
		objects.push(new Line(new ConstantPath(new Quaternion(0, -i, i, j)), new ConstantPath(new Quaternion(
			0, i, i, j)), "red", 2));
	}
}
...
for (var i = -4; i <= 4; i += 8) {
	for (var j = -4; j <= 4; j += 8) {
		...
		objects.push(new Line(new ConstantPath(new Quaternion(0, -i, i, j)), new ConstantPath(new Quaternion(
			0, i, i, j)), "green", 2));
		for (var k = 1; k <= 3; k++) {
			objects[objects.length - k].paths[0].pos = objects[objects.length - k].paths[0].pos.applyRotation(
				new Quaternion(1, 1, 1, 0).normalize());
			objects[objects.length - k].paths[1].pos = objects[objects.length - k].paths[1].pos.applyRotation(
				new Quaternion(1, 1, 1, 0).normalize());
		}
	}
}
for (var i = -4; i <= 4; i += 8) {
	for (var j = -4; j <= 4; j += 8) {
		...
		for (var k = 1; k <= 3; k++) {
			objects[objects.length - k].paths[0].pos = objects[objects.length - k].paths[0].pos.applyRotation(
				new Quaternion(1, 1, 0, 1).normalize());
			objects[objects.length - k].paths[1].pos = objects[objects.length - k].paths[1].pos.applyRotation(
				new Quaternion(1, 1, 0, 1).normalize());
		}
	}
...
objects.push(new Sphere(new OrbitPath(new ConstantPath(new Quaternion(0, 0, 0, 0)), new Quaternion(
	0, 0, 0, 0.2), new Quaternion(60, 0, 1, 0).normalize()), 0.05, getRandomColor()));
objects.push(new Sphere(new OrbitPath(new ConstantPath(new Quaternion(0, 0, 0, 0)), new Quaternion(
	0, 0, 0, 0.4), new Quaternion(80, 0, 1, 0).normalize()), 0.05, getRandomColor()));
objects.push(new Sphere(new OrbitPath(new ConstantPath(new Quaternion(0, 0, 0, 0)), new Quaternion(
	0, 0, 0, 0.7), new Quaternion(120, 0, 1, 0).normalize()), 0.05, getRandomColor()));
objects.push(new Sphere(new OrbitPath(new ConstantPath(new Quaternion(0, 0, 0, 0)), new Quaternion(
	0, 0, 0, 1), new Quaternion(160, 0, 1, 0).normalize()), 0.05, getRandomColor()));
objects.push(new Sphere(new OrbitPath(new OrbitPath(new ConstantPath(new Quaternion(0, 0, 0, 0)),
	new Quaternion(0, 0, 0, 1), new Quaternion(160, 0, 1, 0).normalize()), new Quaternion(0, 0, 0,
	0.1), new Quaternion(30, 0, 1, 0).normalize()), 0.03, "grey"));
objects.push(new Sphere(new OrbitPath(new OrbitPath(new ConstantPath(new Quaternion(0, 0, 0, 0)),
	new Quaternion(0, 0, 0, 1), new Quaternion(160, 0, 1, 0).normalize()), new Quaternion(0, 0, 0,
	0.15), new Quaternion(40, 0, 1, 0).normalize()), 0.02, "grey"));
for (var i = 0; i <= 100; i++) {
